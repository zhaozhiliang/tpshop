    ...
    */
    ...
    public function set_pwd(){
    	...
    	if(IS_POST){
    		...
    		if($check['is_sure']==1){
    			$user = get_user_info($check['email'],1);
    			M('users')->where("user_id=".$user['user_id'])->save(array('password'=>md5($password)));
    ...
    public function send_forget_code(){
    	$sendToken = I('post.sendToken');
    	$type = I('post.type');
    	$username = I('post.username');
    	$userinfo = M('users')->where("email='$username' or mobile='$username'")->find();
    	$token = encrypt($username.C('AUTH_CODE'));
    	if($userinfo){
    		if($type=='email'){
    			$email = $userinfo['email'];
    			$code =  rand(1000,9999);
    			$send = session('forget_code');
    			if($send && $send['time'] > time() && $send['email'] == $userinfo['email']){
    				exit(json_encode(array('status'=>-2,'msg'=>'验证码已发送，请检查')));
    			}
    			//发送电子邮件验证码
    			$send2 = send_email($email,'验证码','您好，你的验证码是：'.$code);
    			if($send2){
    				$info['code'] = $code;
    				$info['email'] = $email;
    				$info['time'] = time()+300;
    				$info['is_sure'] = 0 ;
    				session('forget_code',$info);
    				exit(json_encode(array('status'=>1,'msg'=>'验证码已发送，请注意查收')));
    			}else{
    				exit(json_encode(array('status'=>-1,'msg'=>'邮箱验证码发送失败')));
    			}
    		}
    		if($type == 'phone'){
    			$config = F('sms','',TEMP_PATH);
    			...
    			//发送手机验证码
    			$send2 = sendSMS($userinfo['mobile'],'您好，你的验证码是：'.$code);
    			if($send2){
    				$info['code'] = $code;
