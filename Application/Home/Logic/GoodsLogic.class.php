...
class GoodsLogic extends RelationModel
{
   ...
   {       
        ...
        {                  
            ...
            $list_brand[$k]['href'] = urldecode(U("/Home/Goods/$action",$filter_param,'')); 
        }
   ...
   {       
        ...
        {                          
           ...
           $list_spec[$spec_item[$v]['spec_id']]['item'][] = array('key'=>$spec_item[$v]['spec_id'],'val'=>$v,'item'=>$spec_item[$v]['item'],'href'=>urldecode(U("/Home/Goods/$action",$filter_param,''))); 
        }      
   ...
   public function get_filter_attr($goods_id_arr = array(),$filter_param,$action, $mode = 0)
   {
        ...
        foreach($goods_attr as $k => $v)
        {
           ...
           if(!empty($old_attr))
             ...
             $list_attr[$v['attr_id']]['attr_value'][] = array('key'=>$v['attr_id'],'val'=>$v['attr_value'],'attr_value'=>$v['attr_value'],'href'=>urldecode(U("/Home/Goods/$action",$filter_param,'')));
...
{ 
    ...
    for($i = 0; $i < $c; $i++)
    {
        ...
        if($i == 0)                
            $parr[] = array('value'=>"{$end}以下",'href'=>urldecode(U("/Home/Goods/$action",$filter_param,'')));    
        elseif($i == ($c-1))  
            $parr[] = array('value'=>"{$end}以上",'href'=>urldecode(U("/Home/Goods/$action",$filter_param,'')));
        else    
            $parr[] = array('value'=>"{$start}-{$end}",'href'=>urldecode(U("/Home/Goods/$action",$filter_param,'')));        
    }    
    return $parr;
}
/**
...
function get_filter_menu($filter_param,$action)
{
    ...
    if(!empty($filter_param['brand_id']))
    {
        ...
        $brand['href'] = urldecode(U("/Home/Goods/$action",$tmp,''));
        $menu_list[] = $brand;
    }
    // 规格
    if(!empty($filter_param['spec']))
    {
       ...
       foreach ($spec_group as $k => $v)
       {
            ...
            $spec_menu['href'] = urldecode(U("/Home/Goods/$action",$tmp2,''));
            $menu_list[] = $spec_menu;
       }
    }
    // 属性
    if(!empty($filter_param['attr']))
    {
       ...
       foreach ($attr_group as $k => $v)
       {
            ...
            $attr_menu['href'] = urldecode(U("/Home/Goods/$action",$tmp2,''));             
            $menu_list[] = $attr_menu;
       }       
    }     
    // 价格
    if(!empty($filter_param['price']))
    {
            ...
            $price_menu['href'] = urldecode(U("/Home/Goods/$action",$filter_param,''));             
