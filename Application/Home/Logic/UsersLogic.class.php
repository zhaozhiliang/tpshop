    ...
    public function reg($username,$password,$password2){
     ...
     public function get_info($user_id){
        ...
        if(!$user_info)
         ...
         $user_info['coupon_count'] = M('coupon_list')->where(array('uid'=>$user_id))->count(); //获取优惠券列表         
...
//        $type = I('get.type',0);
     ...
     * @param $user_id  用户id
     */
    public function get_goods_colletc($user_id){
        ...
        $sql = "SELECT c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.original_img FROM __PREFIX__goods_collect c ".
            "LEFT JOIN __PREFIX__goods g ON g.goods_id = c.goods_id ".
     ...
     * @return mixed
     */
    public function get_comment2($user_id,$status){
        //获取已收货订单
        $order_list = M('order')->where("user_id = {$user_id}".C('WAITCCOMMENT'))->getField('order_id',true);
        if(empty($order_list))
            return array('status'=>1,'msg'=>'没有数据');
        $map['order_id'] = array('in',$order_list);

        //待评价数量
        $map['is_comment'] = 0;
        $count1 = M('order_goods')->where($map)->count();

        //已评价数量
        $map['is_comment'] = 1;
        $count2 = M('order_goods')->where($map)->count();

        if($status > 0){
            //已评论
            $page = new Page($count2);
            $sql = "SELECT o.order_sn,og.goods_id,og.order_id,o.add_time AS order_add_time,og.goods_name,g.original_img,c.comment_id,c.content,c.username,c.add_time,og.is_comment,c.deliver_rank,c.goods_rank,c.service_rank FROM __PREFIX__order_goods og".
                " LEFT JOIN __PREFIX__comment c ON og.goods_id = c.goods_id".
                " LEFT JOIN __PREFIX__goods g ON og.goods_id = g.goods_id".
                " LEFT JOIN __PREFIX__order o ON o.order_id = og.order_id".
                " WHERE og.order_id IN (".implode(',',$order_list).") AND og.is_comment = 1".
                " ORDER BY o.add_time DESC LIMIT {$page->firstRow},{$page->listRows}";

        }else{
            //待评论
            $page = new Page($count1);
            $sql = "SELECT o.order_sn,og.goods_id,og.order_id,og.goods_name,g.original_img,o.add_time AS order_add_time,og.is_comment FROM __PREFIX__order_goods og".
                " LEFT JOIN __PREFIX__goods g ON og.goods_id = g.goods_id".
                " LEFT JOIN __PREFIX__order o ON o.order_id = og.order_id".
                " WHERE og.order_id IN (".implode(',',$order_list).")  AND og.is_comment = 0".
                " ORDER BY o.add_time DESC LIMIT {$page->firstRow},{$page->listRows}";
        }
        $show = $page->show();
        ...
        $return['show'] = $show; //分页
        $return['count1'] = $count1;  //待评价数量
        $return['count2'] = $count2; //已评价数量
     ...
     * @return mixed
     */
    public function get_comment($user_id,$status){
        //获取已收货订单
         $is_comment = I('status',0);
        // 待评价
         $sql ="select count(1) as comment_count from __PREFIX__order_goods as og 
         left join __PREFIX__order as o on o.order_id = og.order_id and o.order_status=2
         left join __PREFIX__comment as c on c.order_id = og.order_id and  c.goods_id = og.goods_id 
         where c.user_id = $user_id  and og.is_comment = 0";
         $comment = M()->query($sql);
         $count1 = $comment_count = $comment[0][comment_count];
          
         // 已评价
         $sql ="select count(1) as comment_count from __PREFIX__order_goods as og 
         left join __PREFIX__order as o on o.order_id = og.order_id and o.order_status=2
         left join __PREFIX__comment as c on c.order_id = og.order_id and  c.goods_id = og.goods_id 
         where c.user_id = $user_id  and og.is_comment = 1";
         $comment = M()->query($sql);
         $count2 = $comment_count = $comment[0][comment_count];         
        ...
        if($status > 0){
            ...
            left join __PREFIX__order as o on o.order_id = og.order_id and o.order_status=2
            left join __PREFIX__comment as c on c.order_id = og.order_id and  c.goods_id = og.goods_id 
            where c.user_id = $user_id  and og.is_comment = 1 order by o.order_id desc  LIMIT {$page->firstRow},{$page->listRows}";                        
            ...
            left join __PREFIX__order as o on o.order_id = og.order_id and o.order_status=2
            left join __PREFIX__comment as c on c.order_id = og.order_id and  c.goods_id = og.goods_id 
            where c.user_id = $user_id  and og.is_comment = 0 order by o.order_id desc  LIMIT {$page->firstRow},{$page->listRows}";            
        }
        $show = $page->show();
        $comment_list = M()->query($sql);       
        $return['status'] = 1;
     ...
     * @return bool
     */
    //public function add_comment($order_id,$goods_id,$user_email,$nickname,$user_id = 0){
    ...
    public function cancel_order($user_id,$order_id){
        ...
        //有余额支付的情况
        if(($log['user_money'] == $order['user_money'] && $order['user_money'] > 0) || ($log['integral'] == $order['integral'] && $order['integral'] > 0)){
     ...
     * @param $id   订单id
     */
    public function confirm_order($user_id,$id){        
        ...
        if($order['pay_code'] == 'cod')  // 如果是到货付款 的 收货确定之后 变成已付款
