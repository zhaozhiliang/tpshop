...
class IndexController extends BaseController {
    ...
    public function index(){
        ...
        $this->pushVersion();
        $act_list = session('act_list'); //all 获取权限

        $menu_list = $this->getRoleMenu($act_list); //获取权限列表


        $this->assign('menu_list',$menu_list);
        $admin_info = getAdminInfo(session('admin_id'));  //获取管理员的信息
		$order_amount = M('order')->where(array('order_status'=>0))->count(); //获取订单的数量
    ...
    public function welcome(){
    	$this->assign('sys_info',$this->get_sys_info());  //获取服务器的系统信息
    ...
    {
		//var_dump($_SESSION['isset_push']); //1 是这样的值
